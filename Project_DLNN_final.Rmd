...
# add the path to the function here
...
## Independent and Dependent Variables Setup
...
```{r Training Vs. Validation, echo=TRUE, results='markup', cache=TRUE}
input_dim <- 1  
hidden_dim <- 16
output_dim <- 1
...
We have also used AI in our reflection parts to paraphrase our text to make it more readable and concise. Since we are not English native speakers, we found using AI to improve the message we are trying to convey to be effective.

# Tensorflow's Recurrent Neural Network (RNN)

## TF model Definition & Manual Weight Import
...
for (epoch in 1:tf_epochs) {
  ...
  # --- 2. Sum of gradients (for comparison, L2 norm)
  grads <- train_result$grads
  grad_sums_tf[epoch] <- sqrt(sum(sapply(grads, function(g) {
...
## Comparison Summary of Standard and TF RNN

```{r, echo=TRUE, results='markup'}

train_losses <- loss_df$Loss[loss_df$Type == "Train"]
val_losses <- loss_df$Loss[loss_df$Type == "Validation"]

mean_summary <- tibble::tibble(
  Metric = c("Train MSE", "Validation MSE", "Gradient Sum L2 Norm"),
  
  `Base R (mean)` = c(
    round(mean(train_losses), 4),
    round(mean(val_losses), 4),
    round(mean(grad_sums_r), 4)
  ),
  
  `Base R (epoch 100)` = c(
    round(train_losses[100], 4),
    round(val_losses[100], 4),
    round(grad_sums_r[10], 4)  # because we saved every 10 epochs
  ),
  
  `TF (mean)` = c(
    round(mean(tf_train_reported_mse_epoch), 4),
    round(mean(tf_val_reported_mse_epoch), 4),
    round(mean(grad_sums_tf), 4)
  ),
  
  `TF (epoch 100)` = c(
    round(tf_train_reported_mse_epoch[100], 4),
    round(tf_val_reported_mse_epoch[100], 4),
    round(grad_sums_tf[100], 4)
  )
)

knitr::kable(mean_summary, caption = "Summary of Training and Validation Metrics")

```

-   Our MSE values from the base R and TensorFlow implementations converged fairly closely by the 100th epoch, especially compared to where they started. However, small differences remained, largely due to the way TensorFlow handles gradient computation. TensorFlow’s GradientTape efficiently computes gradients, even for complex neural networks with millions of parameters, and is highly optimized for numerical stability. In contrast, our manual gradient implementation in base R is more error-prone and less efficient. This is reflected in the differences in the sum of gradients between the two models, despite our earlier use of gradient checking. These discrepancies highlight the reliability and robustness of TensorFlow’s automatic differentiation.

-   Another clear observation was TensorFlow’s significantly faster training performance. While base R processed each epoch sequentially and without hardware acceleration, TensorFlow leveraged optimized computation graphs and could run on both CPUs and GPUs, greatly accelerating training, even with a relatively simple model.
...
-   **Loss function definition:**\
    Our course defined the loss as $0.5 \times (y_{\text{true}} - \hat{y})^2$, whereas TensorFlow's built-in loss functions typically compute the mean squared error $\text{mean}((y_{\text{true}} - y_{\text{pred}})^2)$. We addressed this mismatch by defining a custom loss function in TensorFlow that explicitly matched the R formulation. However, for reporting we used TF's custom loss function as this gives us comparison and interpretability.
...
Hand-coding neural networks from scratch is of course an enriching experience as we get involved with almost every detail of the model. It is a real challenge though, as designing a neural network requires extensive debugging and continuous problem-solving to different kind of problems, not only related to the architecture itself but also to data handling and preprocessing.

As we have mentioned throughout the project, the role of AI is incredibly significant as a time-saving and problem-solving tool, especially when facing unfamiliar challenges. However, AI remains a tool whose effectiveness depends entirely on the quality of the prompts provided. We have found out that crafting clear, context-rich, and precise prompts is essential to obtaining valuable solutions. This could only be achieved if the user has the sufficient knowledge to filter, guide and interpret AI’s assistance effectively.
