---
title: "Project Deep Learning & Neural Networks"
author: "Udval Oyunsaikhan & Andres Cadena"
date: "2025-05-24"
output: pdf_document
---
...
```{r Setting up the independent and dependent variables}
lookback <- 24
...
One of the biggest challenges we faced was debugging, especially to know how to deal with vanishing and exploding gradients. Initially, our model was getting either exploding gradients or vanishing ones. To address this, we started to experiment with different activation and initilization functions. At first, we tried using ReLU activation function with He initialization, as we remembered from the tutorials that those were widely recommended fuctions to use nowadays. However, as the professor constantly emphasized, the agorithms and fuctions used are also problem dependent. You must try which one works the best for your specific data and task.

After doing some research, we got to the conclusion that when dealing with negative values Tanh was more appropiate as an activation fuction. Along with it, we applied Xavier initialization. This change was groundbreaking; we immediately saw a huge improvement in performance. 

Nevertheless, our MSE remained around thousands, which was far from acceptable. This led us to look the data closely again. At the beginning, we indentified some outliers, but we understimated their impact. On closer inspection, we reaized these extreme values might be harming the trianing process. We then tried a strategy that AI recommended us: capping the outliers (replacing extreme prices with a more reasonable ceiling). This was indeed a game-changer, since we were able to achieve a really low MSE, or at least one acceptable compared to our initial attempts.
